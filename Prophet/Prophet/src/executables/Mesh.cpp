...
Mesh::Mesh(std::string filename)
{
	...
	if (m_texCoords.size() >= m_vertices.size())
	{
		...
		{
			glm::vec3 deltaPos1 = m_vertices[i + 1] - m_vertices[i];
			glm::vec3 deltaPos2 = m_vertices[i + 2] - m_vertices[i];

			glm::vec2 deltaUV1 = m_texCoords[i + 1] - m_texCoords[i];
			glm::vec2 deltaUV2 = m_texCoords[i + 2] - m_texCoords[i];
			...
			m_bitangents[i + 1] = bitangent;
			m_bitangents[i + 2] = bitangent;
		}
